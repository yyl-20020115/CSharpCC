...
public class BitSet
{
    ...
    public static long[] Convert(ulong[] data)
    {
        long[] result = new long[data.Length];
        for (int i = 0; i < data.Length; i++)
            result[i] = unchecked((long)data[i]);
        return result;
    }
    ...
    public long[] ToLongArray() => Convert(_data);
    public ulong[] ToULongArray()
    {
        ulong[] result = new ulong[this._data.Length];
        Array.Copy(this._data,result,this._data.Length);
        return result;
    }
